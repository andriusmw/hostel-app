...
export default async function SingleNewFiltered({ params }: SingleNewProps) {
  const { selectedCategory, setSelectedCategory } = useCategory();

  
    //Con el contexto hemos guardado la categoría seleccionada porque ahora la necesitamos
    //para volver a cargar las noticias seleccionadas y después comparar una a una la que le coincida
    //la "url de la noticia" que usamos como identificador. 
  // usa la ENDPOINT del service para cargarlo todo pasándole la categoría
  
    const decodedUrl = decodeURIComponent(params.id);
    // saca la "url de la noticia", que le hemos pasado, está en el params.id
  
    let article: Article | null = null; 
    let error: string | undefined;
  
    try {
      const res = await fetch(`/api/news?category=${encodeURIComponent(selectedCategory)}`, { next: { revalidate: 3600 } });
          //Saca de nuevo la lista de todas las noticias para comparar abajo. usando la categoría seleccionada
